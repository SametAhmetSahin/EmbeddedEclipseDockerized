...
WORKDIR /app
COPY . /app
#RUN echo "eclipsedockerized" > /etc/hostname
RUN apt update
RUN echo -e "8\n20" apt install -y --no-install-recommends libxcursor1 libxinerama1 libxrandr2 libxi6 libswt-gtk-4-jni openocd gcc-arm-none-eabi usbutils build-essential libcanberra-gtk3-0 gdb-multiarch ssh xterm
# add new user
